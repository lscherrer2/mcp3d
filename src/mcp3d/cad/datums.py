...
def build_datum_plane(
    ...
    if kind == "explicit":
        return Plane(
            ...
            x_dir=values.vector3(definition.get("x_dir"), f"{identifier}.x_dir"),
            z_dir=values.vector3(definition.get("normal"), f"{identifier}.normal"),
        )
    if kind == "offset_from_face":
        ...
        return planes[support].offset(values.length(definition.get("offset", 0), f"{identifier}.offset"))
    if kind == "rotate_about":
        ...
        angle = values.angle(definition.get("angle"), f"{identifier}.angle")
