...
def export_revision(part_id: str, revision: Revision, artifact_root: Path, formats: list[str] | None) -> dict[str, Any]:
    """Write requested STEP/STL files and return their exact local paths."""
    artifact_dir = artifact_root / part_id / f"r{revision.number}"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    exported: dict[str, str] = {}
    for format_name in formats or ["step"]:
        destination = artifact_dir / f"{revision.number}.{format_name.lower()}"
        if format_name == "step":
    ...
    return {"status": "ok", "part_id": part_id, "revision": revision.number, "artifacts": exported}
